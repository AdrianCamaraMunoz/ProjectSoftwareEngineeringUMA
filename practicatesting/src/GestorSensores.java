public class GestorSensores {
	...
	private static final int MAX_SENSORES = 100;
	...
	public GestorSensores() {
		listaSensores = new ISensorTemperatura[GestorSensores.MAX_SENSORES];
		listaIntentosDesconectados = new int[GestorSensores.MAX_SENSORES];
		primerIndiceLibre = 0;
	}
	...
	public static int max_sensores(){
		return MAX_SENSORES;
	}
	...
	public void introducirSensor(ISensorTemperatura sensor) {
		if (primerIndiceLibre >= GestorSensores.MAX_SENSORES) {
	...
	public void contactarSensores() {
		for (int i = primerIndiceLibre - 1; i >= 0; i--) {
			...
			if (!sensor.disponible()) {
				...
				if (listaIntentosDesconectados[i] >= GestorSensores.MAX_INTENTOS) {
					borrarSensor(i);
				}
			} else {
